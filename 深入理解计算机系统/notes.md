...
虽然系统的具体的实现方式随着时间发生了变化，但是系统的基本概念没有发生变化
* 编译系统对程序的执行过程
![](images/0101.png)

### 1.1 信息就是位+上下文

* 文本文件与二进制文件：由ASCII码构成的文件是文本文件，其余的是二进制文件以字符序列的方式存储与文件中
* C语言的特点：
  * C语言于Unix操作系统关系密切
  * C语言小而简单
  * C语言是为实践目的设计的
...
* 优化程序性能：编译器如何优化代码，switch一定比if else高效？while循环比for循环更有效？等等求和的时候用一个本地变量会比指针引用更有效
* 理解链接时出现的错误：最令人困惑的程序错误往往都与链接器操作有关，无法解析引用，命令行上排列库的顺序的影响等等
* 避免安全漏洞：缓冲区溢出错误是造成安全漏洞的主要原因，理解程序中堆栈的工作原理以及可能带来的问题
...
* I/O设备：如键盘，鼠标，显示器与磁盘（通过控制器/适配器（封装方式不同，前者是系统主板上的芯片组或设备本身，后者是主板插槽上的卡）与总线相连）
* 主存：随机存取存储器，存储这运行的程序及其所需要的数据，DRAM
* 处理器：有ALU，寄存器以及PC组成，注意指令集和微体系结构的区别

#### 1.4.2 运行hello程序

* shell程序读取键盘的输入`./hello`经寄存器至主存；将程序内容从磁盘读入到主存（借助直接存储器不需要经过寄存器）；将hello，world从内存复制到寄存器再到显示器
![](images/0103.png)
...
* 三级缓存机制，sram，局部性原理：程序具有访问局部区域里的代码和数据的趋势
![](images/0106.png)
...
* 文件是对I/O设备的一种抽象，虚拟内存是对磁盘I/O的抽象表示，进程则是对处理器，内存和I/O设备的抽象表示。
#### 1.7.1 进程

* 进程是操作系统对一个正在运行的程序的一种抽象。
* 操作系统保持和跟踪进程所有的状态信息，这个状态信息就是上下文
* 进程之间的上下文切换，用户代码之间是内核代码。


#### 1.7.2 线程

* 一个进程可以由多个线程执行单元组成，每个线程都运行在进程的上下文中，共享代码和全局数据。
#### 1.7.3 虚拟内存

* 虚拟地址空间的分布（由上到下地址减小）：内核代码，用户栈，共享库内存映射区，运行时堆，读/写数据，只读的代码和数据。
#### 1.7.4 文件
